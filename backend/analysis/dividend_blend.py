...
merged_df = pd.merge(
    df2[['symbol', 'date', 'calendarYear', 'payoutRatio', 'dividendYield']], 
...
# Function to filter and compute median dividend growth
def filter_and_compute_median_growth(group):
    ...
    if len(recent_years) == 10 and all(recent_years['dividendsperShareGrowth'] > 0) and all(recent_years['revenueGrowth'] > 0):
        median_growth = recent_years['dividendsperShareGrowth'].median()
        recent_years = recent_years.copy()  # Avoid modifying a slice of the original DataFrame
        recent_years['medianDividendGrowth'] = median_growth  # Add new column
        return recent_years
    return None

# Apply function to each group
filtered_stocks = filtered_df.groupby('symbol').apply(filter_and_compute_median_growth)

# Remove groups that returned None
filtered_stocks = filtered_stocks.dropna(subset=['medianDividendGrowth'])

# Keep only data for the selected year
filtered_df = filtered_stocks[filtered_stocks['calendarYear'] == selected_year]

filtered_df['dividendNow'] = filtered_df['dividendYield'] * filtered_df['stockPrice']
filtered_df['medianDividendGrowthPercent'] = filtered_df['medianDividendGrowth'] * filtered_df['dividendNow']
# Remove 'revenueGrowth' from the DataFrame
filtered_df = filtered_df.drop(columns=['revenueGrowth', 'dividendsperShareGrowth', 'medianDividendGrowth'])

# Reorder columns to have 'name' first
columns = ['name'] + [col for col in filtered_df.columns if col != 'name']
...
filtered_df = filtered_df[
    ...
    (filtered_df['payoutRatio'] > 0) &
    (filtered_df['payoutRatio'] < 0.6) 
]
...
filtered_df.sort_values(by='dividendYield', ascending=False, inplace=True)
