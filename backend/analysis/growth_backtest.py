...
# merged_df = merged_df[merged_df['symbol'] == 'AAPL']
# print(merged_df)

def find_max_future_price(group):
    group = group.sort_values(by='calendarYear')
    future_years = group[(group['calendarYear'] > selected_year) & (group['calendarYear'] <= selected_year + 5)]
    if len(future_years) > 0:
        max_price = future_years['stockPrice'].max()
        return max_price
    return np.nan
...
merged_df['maxFuturePrice'] = merged_df.groupby('symbol')['stockPrice'].transform(
    lambda x: find_max_future_price(merged_df[merged_df['symbol'] == x.name])
)
...
merged_df['Change'] = merged_df['maxFuturePrice'] / merged_df['stockPrice']
...
columns_to_display = [
    ...
    'revenueGrowth', 'Score', 'stockPrice', 'maxFuturePrice', 'Change'
]
